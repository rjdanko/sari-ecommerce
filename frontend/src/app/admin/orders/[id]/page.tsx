      ...
      {order.user && (
        <div className="rounded-2xl bg-white border border-gray-100 shadow-sm p-6 space-y-2">
          ...
          <dl className="grid grid-cols-2 gap-3 text-sm">
            ...
            <div><dt className="text-gray-500">Email</dt><dd className="font-medium text-gray-900">{order.user.email}</dd></div>
          </dl>
          <Link href={`/admin/users/${order.user.id}`} className="inline-block text-xs text-red-700 hover:underline">View user →</Link>
        </div>
      )}
