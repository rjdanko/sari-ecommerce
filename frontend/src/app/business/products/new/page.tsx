...
export default function NewProductPage() {
  ...
  useEffect(() => {
    if (name && !sku) {
      const generated = name
        ...
        .replace(/[^A-Z0-9]/g, '')
        .slice(0, 8);
      setSku(`SARI-${generated}-${Math.random().toString(36).slice(2, 6).toUpperCase()}`);
    }
  }, [name]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await api.post('/api/business/products', formData, {
  ...
  return (
    <div className="px-8 py-6">
      ...
      <div className="mb-8 flex items-center gap-4">
        ...
        <div>
          <h1 className="font-display text-2xl tracking-tight text-gray-900">Add New Product</h1>
          <p className="mt-0.5 text-sm text-gray-500">Fill in the details to list a new product.</p>
        </div>
      </div>
          ...
          <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
            <div className="mb-4 flex items-center justify-between">
              <div>
                ...
                <p className="mt-0.5 text-xs text-gray-400">Add options like Size, Color to auto-generate variants.</p>
              </div>
              <button
                type="button"
            ...
            {optionCategories.length === 0 ? (
              <div className="rounded-xl border border-dashed border-gray-200 bg-gray-50/50 px-6 py-8 text-center">
                ...
                <p className="mt-2 text-sm text-gray-400">No options added yet.</p>
                <p className="text-xs text-gray-400">Products without options will have a single default variant.</p>
              </div>
            ) : (
              <div className="space-y-4">
